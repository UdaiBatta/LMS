...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f'Created course: {dbms_course.title} ({dbms_course.code})')
            )
        else:
            self.stdout.write(f'Course already exists: {dbms_course.title} ({dbms_course.code})')
        
        # Create a sample quiz for DBMS
        quiz, created = Quiz.objects.get_or_create(
            title="DBMS Fundamentals Quiz",
            course=dbms_course,
            defaults={
                "description": "Test your knowledge of basic database concepts and SQL.",
                "category": "practice",
                "pass_mark": 70,
                "random_order": False,
                "answers_at_end": True,
        ...
        if created:
            ...
            questions_data = [
                ...
                {
                    "content": "Which of the following is a primary key constraint?",
                    "choices": [
                        ("It allows NULL values", False),
                        ("It allows duplicate values", False),
                        ("It uniquely identifies each record", True),
                        ("It can have multiple columns only", False)
                    ],
                ...
                },
                {
                    ...
                    "choices": [
                        ("Process of organizing data to minimize redundancy", True),
                        ("Process of creating backup copies", False),
                        ("Process of indexing tables", False),
                        ("Process of encrypting data", False)
                    ],
        ...
        self.stdout.write('\n' + '='*50)
        self.stdout.write('EXISTING COURSES:')
        self.stdout.write('='*50)
        
        for course in Course.objects.all().order_by('program__title', 'title'):
            self.stdout.write(f'• {course.title} ({course.code}) - {course.program.title}')
        
        self.stdout.write('\n' + self.style.SUCCESS('Sample data creation completed!'))
        self.stdout.write(f'You can now access the DBMS course at: /programs/course/{dbms_course.slug}/')
        self.stdout.write(f'Quiz URL: /quiz/{dbms_course.slug}/quizzes/')
